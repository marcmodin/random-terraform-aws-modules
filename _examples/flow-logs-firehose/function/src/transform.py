...
for k in custom_source_mapping['custom_source_events']['ocsf_mapping'].keys():
    schemas.append(
        custom_source_mapping['custom_source_events']['ocsf_mapping'][k]['schema'])
...
def lambda_handler(event, context):
    ...
    if mapped_events:
        ...
        for eventday in df_eventdays:
            ...
            for ocsf_schema in df_schemas:
                ...
                df_map = pd.json_normalize(
                    df_map['target_mapping'], max_level=0)
                if MULTISCHEMA:
    ...
    if unmapped_events:
        ...
        logger.info('Dropping {} unmapped records.'.format(
            len(unmapped_events)))
